...
bool playAgn(float, float);
int  buyChps(int &, int &);
int  findCrd(int arr[], int size, int val);
void bet(int &, int &);
void bublSrt(int [], int);
void compare(int, int, int, int &, float &, float &);
void showFnl(int, int);
void filSuit(int []);
void filSuit(vector<int> &);
void inpFile();
void outFile(int);
void prntSt(int []);
void prntSt(vector<int>);
void instrct();
void rTable();
void selSrt(vector<int> &, int);
...
int main(int argc, char **argv) {
    ...
    //initial game setup
    inpFile();          // input information from other file
    instrct();      // instructions
    filSuit(allSpds);        // fill all spades
    filSuit(allHrts);        // fill all hearts
    filSuit(allDmnd);        // fill all diamonds
    filSuit(allClbs);        // fill all clubs
    bublSrt(allHrts, suitSz); // sort hearts in descending order
    selSrt(allClbs, suitSz);  // sort clubs in descending order

    //print all 4 suits to see that sorts work
    cout << "Here are all the cards in the deck:" << endl;
    cout << left << setw(10)<< "Spades: "  ; prntSt(allSpds); // print Spades
    cout << left << setw(10)<< "Hearts: "  ; prntSt(allHrts); // print Hearts
    cout << left << setw(10)<< "Diamonds: "; prntSt(allDmnd); // print Diamonds
    cout << left << setw(10)<< "Clubs: "   ; prntSt(allHrts); // print Clubs
    cout << endl;

    //find lucky card
    cout << "Your lucky card is the 7 of spades" << endl;
    cout << "It is currently at index " << findCrd(allSpds, 14, 4) << " of the Spades array" << endl << endl;

    //enter starting money
    pStack = buyChps(pStack, start);
    ...
    do {
        ...
        //initial card values
        pCount=dCount=0;           //start each hand at 0
        ...
        if (pCount > 21) {
            ...
            compare(pCount, dCount, pBet, pStack, pWin, dWin);
        }

        //ask if player wants to play another hand
        again = playAgn(pWin, dWin);

    } while (again);

    //end of game
    showFnl(pStack, start); // display final winnings/losses
    rTable();              // display results table
    outFile(pStack);        // output final stack to other file
...
//play another hand
bool playAgn(float pWin=1, float dWin=1) {
    char cont;
...
//buy initial chips
int buyChps(int &pStack, int &start) {
...
void compare(int pCount, int dCount, int pBet, int &pStack, float &pWin, float &dWin) {
...
void showFnl(int pStack, int start) {
...
void filSuit(int arr[]){
    //fill array with numbers
    //on the 10th iteration, assign values to the face cards
    for (int i=0; i<11; i++) {
        if (i == 10) {
            ...
            arr[i] = i+1; //print out numbered cards
    }
}

//fill the deck with all cards in one suit (vectors)
void filSuit(vector<int> &vect) {
    for (int i=1; i<=11; i++) {
        if (i == 11) {
...
//selection sort
void selSrt(vector<int> &vect, int size) {
    ...
    for (sp=0; sp<size-1; sp++) {
        ...
        for (int index=sp+1; index < size-1; index++) {
            if (vect[index] > minVal) {
                minVal = vect[index];
                minInd = index;
            }
        }
